...
app.get("/random", (req, res) => {
  const randomIndex = Math.floor(Math.random() * jokes.lenth);
...
app.patch("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const existingJoke = jokes.finds.find((joke) => joke.id === id);
  const replacementJoke = {
    id: id,
...
app.delete("/all", (req, res) => {
  ...
  if (userKey === masterKey) {
    ...
    res
    .status(404)
    .json({error: 'you are not authorised to perfom this action.'});
  }
});
